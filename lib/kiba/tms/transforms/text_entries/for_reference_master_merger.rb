...
module Kiba
  module Tms
    module Transforms
      module TextEntries
        class ForReferenceMasterMerger
          def initialize
            @lookup = Tms::Mixins::MultiTableMergeable.get_merge_lookup(
              Tms::TextEntriesForReferenceMaster
            )
            @treatments = Tms::TextEntriesForReferenceMaster.treatment_mergers
              .transform_keys { |key| key.to_s }
              .transform_values { |val| val.new }
          end

          def process(row)
            merge_rows = lookup[row[:referenceid]]
          ...
          def do_merge(row, mergerow)
            ...
            treatment = if treatment_val.blank?
              Tms::TextEntries.for_reference_master_untyped_default_treatment
            else
              treatment_val
            end
