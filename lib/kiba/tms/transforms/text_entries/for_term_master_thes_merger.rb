...
module Kiba
  module Tms
    module Transforms
      module TextEntries
        class ForTermMasterThesMerger
          def initialize
            @lookup = Tms::Mixins::MultiTableMergeable.get_merge_lookup(
              Tms::TextEntriesForTermMasterThes
            )
            @treatments = Tms::TextEntriesForTermMasterThes.treatment_mergers
              .transform_keys { |key| key.to_s }
              .transform_values { |val| val.new }
          end

          def process(row)
            merge_rows = lookup[row[:termmasterid]]
          ...
          def do_merge(row, mergerow)
            ...
            treatment = if treatment_val.blank?
              Tms::TextEntries.for_term_master_thes_untyped_default_treatment
            else
              treatment_val
            end
