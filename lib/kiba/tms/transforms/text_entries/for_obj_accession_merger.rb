...
module Kiba
  module Tms
    module Transforms
      module TextEntries
        # Used in job that has an ObjAccession table as its source
        class ForObjAccessionMerger
          def initialize
            @lookup = Tms::Mixins::MultiTableMergeable.get_merge_lookup(
              Tms::TextEntriesForObjAccession
            )
            @treatments = Tms::TextEntriesForObjAccession.treatment_mergers
          ...
          def do_merge(row, mergerow)
            ...
            treatment = if treatment_val.blank?
              Tms::TextEntries.for_obj_accession_untyped_default_treatment
            else
              treatment_val
            end
