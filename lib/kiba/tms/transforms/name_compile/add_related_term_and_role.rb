    ...
    module Transforms
      module NameCompile
        class AddRelatedTermAndRole
          def initialize(target:, maintype:, mainnamefield:, relnamefield:,
            rolefield: nil)
            @target = target
            @maintype = maintype
            ...
            if rolefield
              @rolefield = ->(row) {
                row[:related_role] = row[rolefield]
              }
            end
          ...
          attr_reader :target, :maintype, :mainnamefield, :relnamefield,
            :rolefield, :namefield, :relbuilder

          def add_relator(row)
            rolefield ? rolefield.call(row) : row[:related_role] =
                                                relbuilder.call(row)
          end
        end
      end
