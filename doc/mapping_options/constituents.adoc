...
The `:termpreferred` value from rows with this treatment will be mapped to `Constituents` `:term_gender`. If there is a `:remarks` value, it is prepared via the following pattern and the result is mapped to `Constituents` `:term_gender_note`.
