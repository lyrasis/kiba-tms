...
This treatment applies to rows where `registrationsetid`, `accessionlotid`, and `acquisitionlot` fields are empty and the `acquisitiontnumber` field is populated.
...
This treatment applies to rows where `registrationsetid`, `accessionlotid`, `acquisitionlot`, and `acquisitionnumber` fields are empty.
