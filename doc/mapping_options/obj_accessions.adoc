...
=== :approval_date_2_treatment
Applies to :approvalisodate2 field in ObjAccession table.

`:approvalisodate1` field is mapped to CS `:acquisitionauthorizerdate` field, but that is a single-valued field.
...
* `:prefer`-- map to `:acquisitionauthorizerdate` field. Drops any `:approvalisodate1` value.
* `:drop` - do not migrate this information

.Related options:
* `:approval_date_2_prefix` -- if treatment involves mapping the value to a note, this is the string prepended to the value to clarify the meaning of the value. Default: "Subsequent approval date: "
